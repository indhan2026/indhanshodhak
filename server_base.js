...
async function sendEmail(to, subject, html) {
  if (!process.env.EMAIL_USER || process.env.EMAIL_PASS==='xxxx xxxx xxxx xxxx') {
    console.log(`[EMAIL SKIPPED - no password set] To: ${to}`);
    return;
  }
  try {
    await mailer.sendMail({ from:`"IndhanShodhak" <${process.env.EMAIL_USER}>`, to, subject, html });
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        app_id: appRow?.id,
        message: 'Application submitted! Verifier will review within 48 hours.'
      });
    } catch(e){
    ...
    try {
      ...
      dbRun(`INSERT INTO user_fuel_applications
        (user_id, applicant_name, applicant_email, vehicle_number, fuel_type, category,
         profession, doc_aadhaar, doc_vehicle_rc, doc_profession_cert, doc_employer_letter,
...
app.get('/api/verify/fuel-applications', requireAuth(['doc_verifier','super_admin']), (req,res) => {
  ...
  let sql = `SELECT a.*, u.mobile FROM user_fuel_applications a JOIN users u ON u.id=a.user_id WHERE a.status=?`;
  const params = [status];
...
app.post('/api/admin/settings', requireAuth(['super_admin']), (req,res) => {
  const allowed=['subscription_price','trial_days','report_expiry_user','report_expiry_owner','rationing_mode','verification_mode','sla_hours','admin_email','razorpay_key_id','razorpay_key_secret','govt_shared_id','govt_shared_pwd','govt_shared_pwd_plain','mapmyindia_token'];
  const updated=[];
...
async function sendOTPEmail(email, otp, name='User') {
  ...
  <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#f9f9f9;border-radius:16px;overflow:hidden">
    ...
    <div style="background:#f5f5f5;padding:12px;text-align:center;font-size:11px;color:#aaa">
      IndhanShodhak · Ahilyanagar, Maharashtra · indhanshodhak@gmail.com
    </div>
  </div>`;

  try {
    await mailer.sendMail({
      from:    `"IndhanShodhak ⛽" <${process.env.EMAIL_USER}>`,
      to:      email,
